# Niryo One motion layer

## What is it?

This repository contains the code to lauch a Niryo One motion layer in docker container. 

## Run it

You will need to setup the host IPs for the niryo-one-sim, niryo-one-control and niryo-one-interface inside the script `./run_container` based on your local configuration.
...
In our run_container.sh example:
  - the container static ip: ```**--ip=10.0.2.195**``` ```**-e ROS_IP=10.0.2.195**```
  - the ROS master pointing to the niryo-one-sim: ```**-e ROS_MASTER_URI=10.0.2.193**``` 
  - the niryo-one-control static ip: ```**--add-host niryo-one-sim:10.0.2.194**``` 
  - the niryo-one-sim static ip: ```**--add-host niryo-one-motion:10.0.2.193**```
