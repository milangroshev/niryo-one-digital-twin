...
MAX_MULTIPLIER = 0.15
MIN_MULTIPLIER = 0.01
DEFAULT_MULTIPLIER = 0.010
STEP_MULTIPLIER = 0.01

LOOP_MULTIPLIER = 0.050
MIN_LOOP_MULTIPLIER = 0.020
MAX_LOOP_MULTIPLIER = 0.50
...
class JointMode:
    ...
    def increase_loop(self):
        self.timer_rate += LOOP_MULTIPLIER
        if self.timer_rate > MAX_LOOP_MULTIPLIER:
            self.timer_rate = MAX_LOOP_MULTIPLIER
        rospy.logwarn("Control-loop value: %s s", str(self.timer_rate))

    def decrease_loop(self):
        self.timer_rate -= LOOP_MULTIPLIER
        if self.timer_rate < MIN_LOOP_MULTIPLIER:
            self.timer_rate = MIN_LOOP_MULTIPLIER
        rospy.logwarn("Control-loop value: %s s", str(self.timer_rate))
    ...
    def increase_speed(self):
        ...
        rospy.logwarn("Movement offset value: %s rad", str(self.multiplier))
    ...
    def process_joy_message(self, joy):
        ...
        if self.axes[AXE_RT] == -1.0:
            self.increase_loop()
        elif self.axes[AXE_LT] == -1.0:
            self.decrease_loop()
