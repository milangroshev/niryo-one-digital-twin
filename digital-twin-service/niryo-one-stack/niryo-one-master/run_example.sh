...
sudo docker run --hostname niryo-one-master -it -d --name master --rm --net host -e ROS_MASTER_URI="http://10.5.98.108:11311" --add-host sim-robot:10.5.98.5 --add-host niryo-one-master:10.5.98.108 --add-host niryo-one-control:10.5.98.108 --add-host niryo-one-motion:10.5.98.108 --add-host niryo-one-interface:10.5.98.108 --add-host niryo-one-dtwin:10.5.98.108 --add-host niryo-one-stack:10.5.98.108 niryo-one-master:latest
